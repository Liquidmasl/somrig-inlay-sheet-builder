    ...
    <template v-else>
      ...
      >
        ...
        >
          ...
          <!-- Per-zone controls (paginated) -->
          <div v-if="activeButton.top.zones[activeTopZone]">

            <!-- VERTICAL: index-card tabs above connected card -->
            <template v-if="layout === 'vertical'">
              <div class="flex items-end justify-between gap-2">
                ...
                <div class="inline-flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <button
                    ...
                    :class="activeButton.top.zones[activeTopZone].type === t
                      ? 'bg-blue-600 text-white'
                      : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'"
                    :aria-label="`Set zone ${activeTopZone + 1} type to ${actionTypeLabel(t)}`"
                    @click="onSetZoneType('top', activeTopZone, t)"
                  >
                    <svg v-if="t === 'single'" viewBox="0 0 24 24" class="w-4 h-4" fill="currentColor"><circle cx="12" cy="12" r="5" /></svg>
                    <svg v-else-if="t === 'double'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><circle :cx="12+8" cy="12" r="5" /><circle :cx="12-8" cy="12" r="5" /></svg>
                    <svg v-else-if="t === 'hold'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><rect x="-3" y="7" width="30" height="10" rx="5" ry="5" /></svg>
                ...
                <div class="flex items-end gap-2">
                  <div class="flex items-end">
                    <button
                      class="px-3 py-1 text-xs font-medium border rounded-tl-md transition-colors relative -mb-px z-10"
                    ...
                    >Icon</button>
                    <button
                      class="px-3 py-1 text-xs font-medium border border-l-0 rounded-tr-md transition-colors relative -mb-px z-10"
                  ...
                  <button
                    ...
                    class="mb-0.5 px-2 rounded-md text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
              ...
              <div class="relative border border-gray-200 dark:border-gray-700 rounded-b-lg rounded-tl-lg bg-white dark:bg-gray-800 p-3 space-y-3">
                <!-- Icon tab content -->
                <template v-if="activeTopZoneTab === 'icon'">
                  <div class="flex items-center gap-2">
                    <button class="flex-1 flex items-center gap-2 px-2 py-1.5 min-h-[36px] rounded-md border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 text-sm text-left transition-colors bg-white dark:bg-gray-800"
                    ...
                    <button v-if="activeButton.top.zones[activeTopZone].icon" class="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove icon" @click="clearIcon('top', activeTopZone)">×</button>
                  </div>
                  <div class="flex items-center gap-3 justify-between">
                    ...
                    <div class="min-w-0 pl-2">
                      ...
                      <input type="color" :value="activeButton.top.zones[activeTopZone].iconColor" class="h-7 w-8 rounded cursor-pointer border border-gray-200 dark:border-gray-700 bg-transparent shrink-0" @input="onUpdateZone('top', activeTopZone, 'iconColor', ($event.target as HTMLInputElement).value)" />
                    </div>
                  </div>
                </template>
                <!-- Label tab content -->
                <template v-else>
                  <input type="text" :value="activeButton.top.zones[activeTopZone].label ?? ''" placeholder="Add label…" class="w-full px-2 py-1.5 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm" @input="onUpdateZone('top', activeTopZone, 'label', ($event.target as HTMLInputElement).value)" />
                  <div class="flex items-center gap-3 justify-between">
                    <div class="flex-4 min-w-0">
                      <div class="flex items-center justify-between mb-1">
                        <label class="text-xs text-gray-500">Size</label>
                        <span class="text-xs text-gray-500">{{ activeButton.top.zones[activeTopZone].labelSize ?? 3 }} mm</span>
                      </div>
                    ...
                    <div class="min-w-0 pl-2">
                      <label class="block text-xs text-gray-500 mb-1">Color</label>
                      <input type="color" :value="activeButton.top.zones[activeTopZone].labelColor ?? activeButton.top.zones[activeTopZone].iconColor" class="h-7 w-8 rounded cursor-pointer border border-gray-200 dark:border-gray-700 bg-transparent shrink-0" @input="onUpdateZone('top', activeTopZone, 'labelColor', ($event.target as HTMLInputElement).value)" />
                    </div>
                  </div>
                  <div v-if="activeButton.top.zones[activeTopZone].icon" class="flex items-center gap-2 flex-wrap">
                    <div class="inline-flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <!-- HORIZONTAL: simple card, both sections side by side -->
            <template v-else>
              <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="flex items-center justify-between gap-2 p-3">
                  <div class="inline-flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <button v-for="t in ACTION_TYPES" :key="t" class="px-2 py-0.5 text-xs font-medium transition-colors" :class="activeButton.top.zones[activeTopZone].type === t ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'" :aria-label="`Set zone ${activeTopZone + 1} type to ${actionTypeLabel(t)}`" @click="onSetZoneType('top', activeTopZone, t)">
                      <svg v-if="t === 'single'" viewBox="0 0 24 24" class="w-4 h-4" fill="currentColor"><circle cx="12" cy="12" r="5" /></svg>
                      <svg v-else-if="t === 'double'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><circle :cx="12+8" cy="12" r="5" /><circle :cx="12-8" cy="12" r="5" /></svg>
                      <svg v-else-if="t === 'hold'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><rect x="-3" y="7" width="30" height="10" rx="5" ry="5" /></svg>
                      <span v-else-if="t === 'none'" class="text-xs">none</span>
                  ...
                  <button v-if="activeTopZone > 0" class="px-2 rounded-md text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove this zone" @click="onRemoveZone('top', activeTopZone)">Remove</button>
                </div>
                <div class="border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700">
                  <div class="p-3 space-y-3">
                    <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Icon</p>
                    <div class="flex items-center gap-2">
                      <button class="flex-1 flex items-center gap-2 px-2 py-1.5 min-h-[36px] rounded-md border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 text-sm text-left transition-colors bg-white dark:bg-gray-800" :title="activeButton.top.zones[activeTopZone].icon ? 'Change icon' : 'Pick icon'" @click="openIconPicker('top', activeTopZone)">
                        <svg v-if="activeButton.top.zones[activeTopZone].icon" viewBox="0 0 24 24" class="w-5 h-5 shrink-0 text-gray-700 dark:text-gray-300" fill="currentColor"><path :d="activeButton.top.zones[activeTopZone].icon!" /></svg>
                        <span class="text-gray-400 text-xs truncate" :class="{ 'text-gray-700 dark:text-gray-300': activeButton.top.zones[activeTopZone].icon }">{{ activeButton.top.zones[activeTopZone].icon ? 'Change icon' : 'No icon — click to add' }}</span>
                      </button>
                      <button v-if="activeButton.top.zones[activeTopZone].icon" class="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove icon" @click="clearIcon('top', activeTopZone)">×</button>
                    </div>
                    <div class="flex items-center gap-3 justify-between">
                      <div class="flex-4 min-w-0"><div class="flex items-center justify-between mb-1"><label class="text-xs text-gray-500">Size</label><span class="text-xs text-gray-500">{{ activeButton.top.zones[activeTopZone].iconSize }} mm</span></div><input type="range" min="0" max="25" step="0.5" :value="activeButton.top.zones[activeTopZone].iconSize" class="w-full accent-blue-600" @input="onUpdateZone('top', activeTopZone, 'iconSize', parseFloat(($event.target as HTMLInputElement).value))" /></div>
                  ...
                  <div class="p-3 space-y-3">
                    <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Label</p>
                    <input type="text" :value="activeButton.top.zones[activeTopZone].label ?? ''" placeholder="Add label…" class="w-full px-2 py-1.5 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm" @input="onUpdateZone('top', activeTopZone, 'label', ($event.target as HTMLInputElement).value)" />
                    <div class="flex items-center gap-3 justify-between">
                      <div class="flex-4 min-w-0"><div class="flex items-center justify-between mb-1"><label class="text-xs text-gray-500">Size</label><span class="text-xs text-gray-500">{{ activeButton.top.zones[activeTopZone].labelSize ?? 3 }} mm</span></div><input type="range" min="1" max="10" step="0.5" :value="activeButton.top.zones[activeTopZone].labelSize ?? 3" class="w-full accent-blue-600" @input="onUpdateZone('top', activeTopZone, 'labelSize', parseFloat(($event.target as HTMLInputElement).value))" /></div>
                      <div class="min-w-0 pl-2"><label class="block text-xs text-gray-500 mb-1">Rotation</label><div class="flex items-center gap-1"><input type="number" min="-360" max="360" :value="activeButton.top.zones[activeTopZone].labelRotation ?? 0" class="h-7 w-10 rounded border border-gray-200 dark:border-gray-700 bg-transparent text-xs text-center shrink-0" @input="onUpdateZone('top', activeTopZone, 'labelRotation', parseFloat(($event.target as HTMLInputElement).value))" /><span class="text-xs text-gray-400">°</span></div></div>
                    ...
                    <div v-if="activeButton.top.zones[activeTopZone].icon" class="flex items-center gap-2 flex-wrap">
                      ...
                      <label class="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" :checked="activeButton.top.zones[activeTopZone].labelShiftIcon ?? false" class="rounded accent-blue-600" @change="onUpdateZone('top', activeTopZone, 'labelShiftIcon', ($event.target as HTMLInputElement).checked)" /><span class="text-xs text-gray-500">Shift icon</span></label>
                    </div>
                  </div>
                </div>
              </div>
        ...
        >
          ...
          <!-- Per-zone controls (paginated) -->
          <div v-if="activeButton.bottom.zones[activeBotZone]">

            <!-- VERTICAL: index-card tabs above connected card -->
            <template v-if="layout === 'vertical'">
              <div class="flex items-end justify-between gap-2">
                <div class="inline-flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <button v-for="t in ACTION_TYPES" :key="t" class="px-2 py-0.5 text-xs font-medium transition-colors" :class="activeButton.bottom.zones[activeBotZone].type === t ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'" :aria-label="`Set zone ${activeBotZone + 1} type to ${actionTypeLabel(t)}`" @click="onSetZoneType('bottom', activeBotZone, t)">
                    <svg v-if="t === 'single'" viewBox="0 0 24 24" class="w-4 h-4" fill="currentColor"><circle cx="12" cy="12" r="5" /></svg>
                    <svg v-else-if="t === 'double'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><circle :cx="12+8" cy="12" r="5" /><circle :cx="12-8" cy="12" r="5" /></svg>
                    <svg v-else-if="t === 'hold'" viewBox="0 0 24 24" class="w-6 h-4" fill="currentColor"><rect x="-3" y="7" width="30" height="10" rx="5" ry="5" /></svg>
                    <span v-else-if="t === 'none'" class="text-xs">none</span>
                ...
                <div class="flex items-end gap-2">
                  ...
                  <button v-if="activeBotZone > 0" class="mb-0.5 px-2 rounded-md text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove this zone" @click="onRemoveZone('bottom', activeBotZone)">Remove</button>
                </div>
              </div>
              <div class="relative border border-gray-200 dark:border-gray-700 rounded-b-lg rounded-tl-lg bg-white dark:bg-gray-800 p-3 space-y-3">
                <template v-if="activeBotZoneTab === 'icon'">
                  <div class="flex items-center gap-2">
                    ...
                    <button v-if="activeButton.bottom.zones[activeBotZone].icon" class="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove icon" @click="clearIcon('bottom', activeBotZone)">×</button>
                  </div>
                  <div class="flex items-center gap-3 justify-between">
                ...
                <template v-else>
                  <input type="text" :value="activeButton.bottom.zones[activeBotZone].label ?? ''" placeholder="Add label…" class="w-full px-2 py-1.5 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm" @input="onUpdateZone('bottom', activeBotZone, 'label', ($event.target as HTMLInputElement).value)" />
                  <div class="flex items-center gap-3 justify-between">
                  ...
                  <div v-if="activeButton.bottom.zones[activeBotZone].icon" class="flex items-center gap-2 flex-wrap">
                    <div class="inline-flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <label class="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" :checked="activeButton.bottom.zones[activeBotZone].labelShiftIcon ?? false" class="rounded accent-blue-600" @change="onUpdateZone('bottom', activeBotZone, 'labelShiftIcon', ($event.target as HTMLInputElement).checked)" /><span class="text-xs text-gray-500">Shift icon</span></label>
                  </div>
                </template>
              </div>
            </template>
            ...
            <template v-else>
              <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="flex items-center justify-between gap-2 p-3">
                  ...
                  <button v-if="activeBotZone > 0" class="px-2 rounded-md text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" title="Remove this zone" @click="onRemoveZone('bottom', activeBotZone)">Remove</button>
                </div>
                <div class="border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700">
                  <div class="p-3 space-y-3">
                    <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Icon</p>
